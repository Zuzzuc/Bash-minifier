...
<br>
<br>`./bash-minifier.sh -f="$HOME/Desktop/test.sh" -o="$HOME/Desktop/output.sh"`
...
<br>
<br />`./bash-minifier.sh < $HOME/Desktop/test.sh > $HOME/Desktop/output.sh`
This will read from STDIN and write to STDOUT; because of the redirection,
this works the same way as the previous example.
<br />
<br>`./bash-minifier.sh -F -f="$HOME/Desktop/test.sh" -o="$HOME/Desktop/output.sh" -p=u-r`
...
Examples: `./bash-minifier.sh -F -f=test.sh -o=STDOUT`<br>`./bash-minifier.sh -F -f=test.sh -o="$HOME/Desktop/output.sh"` 
<br><br><br>
